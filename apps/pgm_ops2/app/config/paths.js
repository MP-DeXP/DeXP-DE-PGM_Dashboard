...
export const ARTIFACT_DIR_URLS = {
    raw_rosetta: new URL('../../artifacts/raw_rosetta/', import.meta.url),
...
export function getBrowserArtifactPath(layer, filename, baseOverride = '/api/pgm-ops2') {
    const normalizedBase = baseOverride.replace(/\/$/, '');
    const apiLayerMap = {
        view_model: 'view-model'
    };
    const layerSegment = apiLayerMap[layer] ?? layer;
