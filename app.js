...
const REQUIRED_FILES = {
    ...
    productTransitionEdge: {
        key: 'product_transition_edge',
        filename: 'pgm_product_transition_edge.csv',
        aliases: ['product_transition_edge.csv']
    },
    returnGravityLoopDetail: {
        key: 'return_gravity_loop_detail',
    ...
    brandRevenueTimeseries: {
        ...
        filename: '_insight_brand_revenue_timeseries.csv',
        aliases: ['brand_revenue_timeseries.csv']
    },
...
const UI_TERM_REPLACEMENTS = [
    ...
    [/\bConvergence\s*Gravity\b/gi, '도착 흐름'],
...
function transformProductTransitionEdgeRows(rows) {
    const src = normalizeCsvRows(rows);
    const groupedByPath = new Map();

    src.forEach((row) => {
        const fromRaw = String(firstDefinedValue(
            row.source_product_id,
            row.from_product_id,
            row.prior_product_id,
            row.entry_product_id,
            row.aa_product_id,
            row.i,
            ''
        )).trim();
        const toRaw = String(firstDefinedValue(
            row.target_product_id,
            row.to_product_id,
            row.next_product_id,
            row.expansion_product_id,
            row.pca_product_id,
            row.j,
            ''
        )).trim();
        if (!fromRaw || !toRaw) return;
        const from = resolveEntityId(fromRaw);
        const to = resolveEntityId(toRaw);
        if (!from || !to || from === to) return;

        const customers = Math.max(0, toNumber(firstDefinedValue(
            row.transition_customer_cnt,
            row.edge_customer_cnt,
            row.transition_customers,
            row.customer_cnt,
            row.customer_count,
            0
        ), 0));
        ...
        if (customers > 0 && Number.isFinite(avgDays)) {
            acc.avg_days_num += avgDays * customers;
            acc.avg_days_den += customers;
        }
        acc.transition_rate = Math.max(
            toNumber(acc.transition_rate, 0),
            toNumber(firstDefinedValue(row.transition_rate, row.edge_rate, 0), 0)
        );
    });

    return Array.from(groupedByPath.values())
        .map((acc) => ({
            source_product_id: acc.source_product_id,
            target_product_id: acc.target_product_id,
            transition_customer_cnt: acc.transition_customer_cnt,
            transition_rate: acc.transition_rate,
            avg_days_to_target: acc.avg_days_den > 0 ? acc.avg_days_num / acc.avg_days_den : null
        }))
        .sort((a, b) => toNumber(b.transition_customer_cnt, 0) - toNumber(a.transition_customer_cnt, 0));
}

function parseIntermediatePathIds(pathValue) {
...
function transformReturnGravityLoopDetailRows(rows) {
    const src = normalizeCsvRows(rows);
    const groupedByLoop = new Map();

    src.forEach((row) => {
        const anchorRaw = String(firstDefinedValue(
            row.return_product_id,
            row.source_product_id,
            row.anchor_product_id,
            row.product_id,
            row.return_product_id,
            row.origin_product_id,
            row.start_product_id,
            row.base_product_id,
            row.aa_product_id,
            ''
        )).trim();
        if (!anchorRaw) return;
        const qualifiedFlag = String(firstDefinedValue(row.qualified_return_flag, row.is_qualified_return, '')).trim();
        const simpleRepeatFlag = String(firstDefinedValue(row.simple_repeat_comparison_flag, row.is_simple_repeat, '')).trim();
        if (qualifiedFlag && qualifiedFlag !== '1' && qualifiedFlag.toLowerCase() !== 'true') return;
        if (simpleRepeatFlag === '1' || simpleRepeatFlag.toLowerCase() === 'true') return;

        const anchor = resolveEntityId(anchorRaw);
        if (!anchor) return;

        const pathIds = parseIntermediatePathIds(firstDefinedValue(row.intermediate_path, row.loop_path, row.return_path, ''));
        const resolvedPath = pathIds
            .map((id) => resolveEntityId(id))
            .filter((id) => id && id !== anchor);
        const firstVia = resolvedPath.length ? resolvedPath[0] : '';
        const lastVia = resolvedPath.length ? resolvedPath[resolvedPath.length - 1] : '';
        if (!firstVia || !lastVia) return;

        const customers = Math.max(0, toNumber(firstDefinedValue(
            row.return_loop_customer_cnt,
            row.loop_customer_cnt,
            row.return_customer_cnt,
            row.customer_cnt,
            row.customer_count,
            1
        ), 0));
        ...
        const key = `${anchor}::${firstVia}::${lastVia}`;
        if (!groupedByLoop.has(key)) {
            groupedByLoop.set(key, {
                product_id: anchor,
                first_via_product_id: firstVia,
                last_via_product_id: lastVia,
        ...
        const acc = groupedByLoop.get(key);
        acc.return_loop_customer_cnt += customers;
        acc.step_count = Math.max(
            toNumber(acc.step_count, 2),
            Math.max(2, toNumber(firstDefinedValue(
                row.intermediate_step_cnt,
                row.loop_length,
                row.path_length,
                row.step_count,
                row.path_hops,
        ...
        acc.distinct_path_count += 1;
    });

    return Array.from(groupedByLoop.values())
...
function rebuildDerivedData() {
    ...
    AppState.data.brandImpactTimeseries = transformBrandImpactTimeseriesRows(raw.brandImpactTimeseries || []);
...
const transformBrandImpactTimeseriesRows = (rows) => {
    return normalizeCsvRows(rows).map((row) => {
        ...
        return {
            ...
            attach_rate_t: asNullableNumber(row.attach_rate_t),
            depth_t: asNullableNumber(row.depth_t),
            stage: withFallback(row.stage, '-'),
            baseline_days: asNullableNumber(row.baseline_days),
            confidence: withFallback(firstDefinedValue(row.confidence, row.confidence_index), '-')
        };
    }).filter((row) => row.as_of_date && row.window_days !== null && (row.bhi !== null || row.bii !== null))
        .sort((a, b) => {
...
const transformBrandImpactDailyPulseRows = (rows) => {
    return normalizeCsvRows(rows).map((row) => ({
        ...
        avg_clv_baseline: asNullableNumber(row.avg_clv_baseline),
        active_customers_t: asNullableNumber(row.active_customers_t),
        repeat_rate_t: asNullableNumber(row.repeat_rate_t),
        attach_rate_t: asNullableNumber(row.attach_rate_t),
        depth_t: asNullableNumber(row.depth_t),
        avg_clv_t: asNullableNumber(row.avg_clv_t),
        clv_t_norm: asNullableNumber(row.clv_t_norm),
        customer_strength_t_norm: asNullableNumber(row.customer_strength_t_norm),
        stage: withFallback(row.stage, '-'),
        scope: withFallback(row.scope, '-')
    })).filter((row) => row.as_of_date && row.daily_bii_pulse !== null)
        .sort((a, b) => (a.as_of_date < b.as_of_date ? -1 : 1));
};

const transformBrandRevenueTimeseriesRows = (rows) => {
    return normalizeCsvRows(rows).map((row) => {
        const windowKey = String(withFallback(row.window_key, '')).trim();
        const matched = windowKey.match(/(\d+)/);
        const windowDays = asNullableNumber(firstDefinedValue(row.window_days, matched ? matched[1] : null));
        return {
            ...
            window_key: windowKey,
            window_days: windowDays,
            revenue_t: asNullableNumber(firstDefinedValue(row.revenue_t, row.revenue, row.revenue_value)),
            currency: withFallback(row.currency, ''),
            scope: withFallback(row.scope, '')
        };
    }).filter((row) => row.as_of_date && row.window_days !== null && row.revenue_t !== null)
        .sort((a, b) => {
            if (a.as_of_date === b.as_of_date) return toNumber(a.window_days, 0) - toNumber(b.window_days, 0);
            return a.as_of_date < b.as_of_date ? -1 : 1;
        });
...
const transformBrandPurchaseDriverTimeseriesRows = (rows) => {
    return normalizeCsvRows(rows).map((row) => {
        const windowKey = String(withFallback(row.window_key, '')).trim();
        const matched = windowKey.match(/(\d+)/);
        const windowDays = asNullableNumber(firstDefinedValue(row.window_days, matched ? matched[1] : null));
        return {
            ...
            attach_rate_t: asNullableNumber(row.attach_rate_t),
            avg_clv_t: asNullableNumber(row.avg_clv_t),
            customers_contribution: asNullableNumber(row.customers_contribution),
            repeat_contribution: asNullableNumber(row.repeat_contribution),
            attach_contribution: asNullableNumber(row.attach_contribution),
            clv_contribution: asNullableNumber(row.clv_contribution),
            momentum_delta_pct: asNullableNumber(row.momentum_delta_pct),
            momentum_state: withFallback(row.momentum_state, ''),
            top_driver_1: withFallback(row.top_driver_1, ''),
            top_driver_2: withFallback(row.top_driver_2, ''),
            hero_summary: withFallback(row.hero_summary, ''),
            stage: withFallback(row.stage, '-'),
            scope: withFallback(row.scope, '-')
        };
    }).filter((row) => row.as_of_date && row.window_days !== null && (row.momentum_t !== null || row.bii_t !== null))
        .sort((a, b) => {
            if (a.as_of_date === b.as_of_date) return toNumber(a.window_days, 0) - toNumber(b.window_days, 0);
            return a.as_of_date < b.as_of_date ? -1 : 1;
        });
...
const preprocessUploadRows = (config, filename, rows) => {
    ...
    if (config.key === REQUIRED_FILES.brandImpactTimeseries.key) {
        return transformBrandImpactTimeseriesRows(rows);
    }
    if (config.key === REQUIRED_FILES.brandImpactDailyPulse.key) {
        return transformBrandImpactDailyPulseRows(rows);
    }
    if (config.key === REQUIRED_FILES.brandRevenueTimeseries.key) {
        return transformBrandRevenueTimeseriesRows(rows);
    }
...
function applyFriendlyUi(root = document.body) {
    ...
    textNodes.forEach((node) => {
        ...
        if (node.parentElement?.closest?.('[data-preserve-abbr="1"]')) return;
    ...
    root.querySelectorAll?.('[title],[placeholder],[aria-label]').forEach((el) => {
        if (el.closest?.('[data-preserve-abbr="1"]')) return;
