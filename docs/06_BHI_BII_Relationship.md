# BHI와 BII(브랜드 구매력)의 관계 (v1.0)

## One-liner
- `BHI`: 브랜드의 4+1 Gravity 구조 건강도
- `BII`: 그 구조 건강도가 기간별 브랜드 구매력으로 나타난 상태
...
## BII가 사용하는 것
`BII`는 `BHI`를 그대로 받아 기간별 브랜드 구매력으로 변환한다.
...
- `Momentum`은 그 구매력이 장기 기준 대비 얼마나 강한지를 본다.
- `Drivers`는 그 Momentum을 무엇이 만들었는지를 본다.

따라서 두 지표는 중복이 아니다.

- `BHI`가 낮으면 구조 병목이 있다는 뜻이다.
- `BII`가 낮으면 최근 브랜드 구매력이 약하다는 뜻이다.
- `BHI`가 높고 `BII`가 낮으면 구조는 있으나 최근 브랜드 구매력이 약한 상태다.
...
- `BHI` → 구조
- `BII` → 구매력
- `Momentum` → 구매력 변화 방향에 대한 상대 강도 해석
- `Drivers` → 변화의 원인

즉 `구조 -> 구매력 -> 변화`의 흐름이다.

`Momentum`은 별도 독립 점수가 아니라 `BII_t / BII_365` ratio 해석 레이어다.

`brand_purchase_driver_timeseries.csv`는 이 해석을 Hero/Drivers UI에 바로 연결하기 위한 공식 파생 산출물이다.
